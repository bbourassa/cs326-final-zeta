...
While our group did have some technical difficulties, the majority of our challenges related to communication. While two of the group members communicated clearly and consistently, one group member was sporadic in communication and contributions. This became confusing when the third member did contribute, as she did not explain her contributions, and at times, committed changes without warning. While some of this was helpful in the short term, a lot of changes had to be made unexpectedly. This was challenging, and it forced us to become very good at communicating, which wasa ultimately a strength.  

Some technical difficulties that we ran into were related to authentication. The authentication was difficult to implement and took a long time to resolve. Due to the authentication issues, some of the routing of pages did not work as intended. Instead of a given page that required an authenticated user checking for a user on the route, and immediately redirecting back to login, we had to implement it so that the page would check authentication on load. While this is clearly not best practice, it does not result in any data leaking. We decided that for the purposes of this project, it was a sufficient solution, specifically because this and the rest of authentication took an entire week to resolve. Another technical difficulty was related to the date/time picking functionality. While there is a visible picker in the Chrome and some other web browsers, in Firefox it is not. Due to these inter-browser compatibility issues, we have found that when the date picker is used, the time that is specified is in GMT. This is an issue that cropped up at the last minute and that we were unable to deeply investigate or resolve.
